...
- 网页悬浮球在用户主动朗读后出现：40px 圆形位于 44×44px 透明命中区内，贴边时仍保留约 32px 可见宽度；悬停或键盘聚焦后完整滑出，播放与定位位于上方，8px 透明连接区保证鼠标可稳定移入快捷按钮；点击后展开紧凑的上一句、暂停/继续、下一句和回到正文控件。
...
browser-model        固定 revision 的 Kokoro v1.1，本地推理（ModelScope 默认，HF 手动备用）
