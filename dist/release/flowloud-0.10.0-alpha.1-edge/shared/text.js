(function attachTextHelpers(global) {
  ...
  const HAN_PATTERN = createUnicodePattern(
    '\\p{Script=Han}',
    'u',
    /[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff]/u
  );
  ...
  function isHanUnit(value) {
    return HAN_PATTERN.test(String(value == null ? '' : value));
  }
  ...
  function segmentWordUnits(text, options) {
    const value = String(text == null ? '' : text);
    if (!value) return [];
    const segmenter = wordSegmenter(options && options.locale);
    if (segmenter) {
      try {
        const output = [];
        for (const entry of segmenter.segment(value)) {
          const segment = String(entry.segment || '');
          if (!segment || entry.isWordLike === false || !hasSpeakableText(segment)) continue;
          // prepareSpeech() removes emoji graphemes before this function is
          // normally called. Keep this guard for callers that provide a custom
          // prepared object so keycaps and emoji-only units never become words.
          if (isEmojiUnit(segment) && !Array.from(graphemeUnits(segment)).some(
            (unit) => !isEmojiUnit(unit.text) && hasSpeakableText(unit.text)
          )) continue;
          output.push({
            text: segment,
            speechStart: entry.index,
            speechEnd: entry.index + segment.length
          });
        }
        if (output.length || !hasSpeakableText(value)) return output;
      } catch (_) {
        // Fall through to the grapheme-safe fallback below.
      }
    }
    return fallbackWordUnits(value);
  }

  function fallbackWordUnits(value) {
    ...
    units.forEach((unit) => {
      ...
      const readable = hasSpeakableText(text) && !isEmojiUnit(text);
      if (!readable) {
        flush();
        return;
      }
      if (isHanUnit(text)) {
        ...
        output.push({
          text,
          speechStart: unit.sourceStart,
          speechEnd: unit.sourceEnd
        });
        return;
      }
      if (!current || current.speechEnd !== unit.sourceStart) {
        flush();
        current = {
          text,
          speechStart: unit.sourceStart,
      ...
      current.text += text;
      current.speechEnd = unit.sourceEnd;
    });
    flush();
    return output;
  }

  function buildSpeechWords(input, options) {
    const prepared = input && typeof input === 'object' &&
      Object.prototype.hasOwnProperty.call(input, 'speechText')
      ? input
      : prepareSpeech(input);
    const speechText = String(prepared && prepared.speechText || '');
    if (!speechText || !hasSpeakableText(speechText)) return [];
    const sourceText = prepared && prepared.sourceText != null
      ? String(prepared.sourceText)
      : speechText;
    const units = segmentWordUnits(speechText, options);
    return units.map((unit, index) => {
      const sourceRange = mapSpeechRange(prepared, unit.speechStart, unit.speechEnd);
      const directMapping = !sourceRange && sourceText === speechText;
      return {
        index,
        text: unit.text,
        kind: 'word',
        speechStart: unit.speechStart,
        speechEnd: unit.speechEnd,
        sourceStart: sourceRange ? sourceRange.sourceStart : (directMapping ? unit.speechStart : null),
        sourceEnd: sourceRange ? sourceRange.sourceEnd : (directMapping ? unit.speechEnd : null)
      };
    });
  }
