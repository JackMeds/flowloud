...
(function backgroundModule(root, factory) {
  ...
  function createChromeTtsManager(chromeApi) {
    ...
    function emit(event, extra) {
      if (!active || active.sourceTabId == null || !chromeApi.tabs?.sendMessage) return;
      const payload = Object.assign({ target: STREAM_EVENT_TARGET, event, type: `tts:stream:${event}`, requestId: active.requestId, playbackId: active.playbackId, sessionId: active.sessionId, clientId: active.clientId, sourceTabId: active.sourceTabId, providerId: 'browser-system', directPlayback: true, transportStreaming: false, progressivePlayback: true }, extra || {});
      try { const sent = chromeApi.tabs.sendMessage(active.sourceTabId, payload); sent?.catch?.(() => {}); } catch (_) {}
    }
    async function request(message) {
      ...
      if (active) { try { tts.stop(); } catch (_) {} emit('cancelled'); }
      const sourceTabId = Number(body.sourceTabId);
      active = { requestId: String(body.requestId || ''), playbackId: String(body.playbackId || ''), sessionId: String(body.sessionId || ''), clientId: String(body.clientId || ''), sourceTabId: Number.isInteger(sourceTabId) && sourceTabId >= 0 ? sourceTabId : null };
      const acceptedRequestId = active.requestId;
      const selectedVoice = String(speech.voice || '').replace(/^browser-system:/, '');
      const options = { enqueue: false, rate: Math.max(.75, Math.min(2, Number(speech.rate || body.playbackRate) || 1)), pitch: Math.max(0, Math.min(2, Number(speech.pitch) || 1)), volume: Math.max(0, Math.min(1, speech.volume == null ? 1 : Number(speech.volume))), onEvent(event) {
        const type = String(event && event.type || '');
        if (type === 'start') emit('started');
        else if (type === 'word' || type === 'sentence' || type === 'marker') emit('boundary', { boundaryType: type, charIndex: Number(event.charIndex) || 0, charLength: Number(event.charLength) || 0, preciseBoundary: type === 'word' });
        else if (type === 'pause') emit('paused'); else if (type === 'resume') emit('resumed');
        else if (type === 'end') { emit('ended', { done: true }); active = null; }
        else if (type === 'cancelled' || type === 'interrupted') { emit('cancelled'); active = null; }
        else if (type === 'error') { emit('error', { error: { code: 'system_voice_error', message: event.errorMessage || '系统语音播放失败。' } }); active = null; }
      } };
      if (selectedVoice) options.voiceName = selectedVoice; if (speech.lang) options.lang = String(speech.lang);
      tts.speak(input, options);
      return { ok: true, requestId: acceptedRequestId, streaming: true, directPlayback: true, transportStreaming: false, progressivePlayback: true, boundaryMode: 'voice-dependent' };
    }
    async function control(message) {
      if (!matches(message)) return { ok: true, count: 0 };
      if (message.type === 'tts:pause') { tts.pause(); return { ok: true, paused: true, count: 1 }; }
      if (message.type === 'tts:resume') { tts.resume(); return { ok: true, resumed: true, count: 1 }; }
      if (message.type === 'tts:cancel') { tts.stop(); emit('cancelled'); active = null; return { ok: true, cancelled: true, count: 1 }; }
