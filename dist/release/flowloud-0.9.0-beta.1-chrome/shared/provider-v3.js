(function providerV3Module(root, factory) {
  ...
  function createBrowserModelProvider(options) {
    ...
    const modelManagement = Object.freeze({
      info: async () => ({ repoId, revision, license: preset?.license || '由模型仓库声明', cached: Boolean(config.downloaded), device: resolvedDevice }),
      download: async (operation) => { await getPipeline(object(operation).onProgress); return { downloaded: true, repoId, revision, device: resolvedDevice, downloadedAt: new Date().toISOString() }; },
      cancel: async () => ({ cancelled: false }),
      delete: async () => { pipelinePromise = null; const keys = typeof caches !== 'undefined' ? await caches.keys() : []; await Promise.all(keys.filter((name) => name.includes(repoId)).map((name) => caches.delete(name))); return { deleted: true }; },
    });
  ...
  function adaptLocalQwen(options) {
    ...
    return normalizeProvider(Object.assign({}, provider, { version: 3, capabilities: Object.assign({}, provider.capabilities, {
