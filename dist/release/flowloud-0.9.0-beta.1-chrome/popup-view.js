(function attachPopupView(global) {
  ...
  function quickSettingsSection(model) {
    ...
    heading.append(element('h2', '', '朗读引擎'), element('span', '', '全局生效'));
    section.append(heading);

    const modeRow = element('div', 'qr-quick-row qr-quick-row-stack');
    const modeCopy = element('span', 'qr-quick-copy');
    modeCopy.append(element('strong', '', '使用方式'), element('small', '', '页面导览只导航与朗读，不会操作网页控件'));
    const mode = element('select', 'qr-quick-select');
    mode.dataset.setting = 'readingMode';
    mode.dataset.focusKey = 'setting:readingMode';
    mode.setAttribute('aria-label', '使用方式');
    [['content', '内容朗读'], ['guide', '页面导览']].forEach(([value, label]) => {
      const option = element('option', '', label); option.value = value; option.selected = value === (settings.readingMode || 'content'); mode.append(option);
    });
    modeRow.append(modeCopy, mode);
    section.append(modeRow);
    if ((settings.readingMode || 'content') === 'guide') {
      const openGuide = button('qr-control qr-control-primary', '打开完整页面导览', 'open-guide');
      section.append(openGuide);
    }
  ...
  function mountPopup(root, model) {
    ...
    header.append(element('h1', 'qr-brand', 'Flowloud / 流声'));
