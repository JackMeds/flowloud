(function settingsSchemaModule(root, factory) {
  ...
  const DEFAULTS = Object.freeze({
    ...
    providerSettings: Object.freeze({
      ...
      'doubao-tts': Object.freeze({
        ...
        appId: '', resourceId: 'seed-tts-2.0', voice: '', responseFormat: 'mp3', rememberKey: false,
      }),
    }),
  ...
  function migrate(input) {
    ...
    const replaceObsoleteBrowserModel = Number(source.schemaVersion || 0) < 6 && !['kokoro-zh'].includes(String(result.providerSettings['browser-model'].modelId || ''));
    ...
    if (replaceObsoleteBrowserModel) result.providerVoices['browser-model'] = DEFAULTS.providerVoices['browser-model'];
    const legacyLocalVoice = splitVoice(object(source.providerVoices)['local-qwen']).voiceId;
    if (legacyLocalVoice) result.providerVoices['local-service'] = namespaceVoice('local-service', legacyLocalVoice);
    if (source.opVoice) result.providerVoices['local-service'] = namespaceVoice('local-service', source.opVoice);
    for (const id of PROVIDER_IDS) {
