...
(function installVoiceStudio() {
  ...
  const transcriptionConsent = element('transcription-consent');
  const skipTranscription = element('skip-transcription');
  ...
  async function transcribeImportItem(item) {
    if (!transcriptionConsent.checked || skipTranscription.checked) {
      item.transcriptionStatus = 'skipped';
      item.status = 'ready';
      renderImports();
      return;
    }
  ...
  transcriptionConsent.addEventListener('change', () => {
    if (transcriptionConsent.checked) skipTranscription.checked = false;
    chrome.storage.local.get('qwenReaderSettings').then((saved) => chrome.storage.local.set({ qwenReaderSettings: Object.assign({}, saved.qwenReaderSettings || {}, { transcriptionConsent: transcriptionConsent.checked }) }));
  });
  skipTranscription.addEventListener('change', () => { if (skipTranscription.checked) transcriptionConsent.checked = false; });
