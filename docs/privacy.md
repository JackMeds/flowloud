...
Flowloud 不含遥测、广告或后台数据收集。为使悬浮播放器在刷新后立即出现，网页内容脚本会在普通 HTTP/HTTPS 页面自动运行，并仅在设备上提取、分段和高亮正文；系统语音与浏览器模型不会上传正文。用户可在浏览器的扩展站点访问设置中限制或关闭网页访问。用户主动选择本地服务时，待朗读文本只会发送到本机的 `localhost`、`127.0.0.1` 或 `::1`；用户主动选择 OpenAI 兼容在线 TTS 或豆包 TTS 时，待朗读文本才会发送给用户配置并授权的 HTTPS 服务，其处理受该服务政策约束。
