...
需要在已有模型缓存上执行完整 Kokoro 下载、离线重启和合成检查时，可追加 `--with-browser-model`。该检查会解析 WAV 的 PCM 信号，静音或削波/交替尖峰等明显失真结果会直接失败；默认设备是兼容性更好的 WASM，默认变体是已验证可发声的 `auto → fp32`。默认发布回归会验证浏览器模型协议与界面，但不会为每次代码修改重复下载大模型。
