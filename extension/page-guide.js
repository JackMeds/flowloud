(function pageGuideController() {
  ...
  document.getElementById('read').onclick = async () => {
    ...
    const control = document.getElementById('read');
    control.disabled = true; control.textContent = '正在开始…';
    try { await message('reader/tts', { text: nodes[index].label }); control.textContent = '重新朗读当前项'; }
    catch (error) { control.textContent = error.message; }
