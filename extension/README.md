...
当前发布版本：`0.5.2`。本版本修复长段落的首句预取与逐词高亮衔接，完善流式暂停，并为词语高亮加入平滑动效。
...
- 长主题采用渐进式提取：首个 API 响应先让队列进入“已就绪”，后续分页在后台补全；导航或重新读取会中止旧请求，不重置正在播放的句子。
...
第二阶段 Provider V2 与流式 TTS 已完成并纳入 `0.5.2` 交付包；Extraction Engine v2
