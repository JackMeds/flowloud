...
test('trusted inline emoji images are projected into extracted forum text', () => {
  const content = loadForumContent();
  const units = content.semanticUnitsFromHtml([
    '<p>正文<img class="emoji" alt="😭"><img class="twemoji extra" alt="😭">续文。</p>',
    '<p>普通<img class="attachment" alt="image350×318 24.5 KB">图片。</p>',
    '<p>伪装<img class="emoji" alt="not-an-emoji">内容。</p>',
    '<p>隐藏<img class="emoji" alt="😭" aria-hidden="true">装饰。</p>'
  ].join(''));
...
test('live DOM extraction restores only trusted emoji alt text omitted by textContent', () => {
  const content = loadForumContent();
  const paragraph = {
    closest: () => null,
    querySelectorAll: () => [],
    cloneNode() {
      ...
      const image = {
        ...
        replaceWith(value) {
          projected = projected.replace('\uFFF0', value);
        }
      };
      const hiddenImage = {
        tagName: 'IMG',
      ...
      return {
        get textContent() { return projected.replace(/[\uFFF0\uFFF1]/gu, ''); },
        querySelectorAll(selector) { return selector === 'img' ? [image, hiddenImage] : []; }
      };
    }
  };
  const root = {
  ...
  assert.deepEqual(content.semanticUnitsFromElement(root).map((unit) => unit.text), ['正文😭续文。']);
});
