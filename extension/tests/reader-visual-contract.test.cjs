...
test('floating entry can be dragged, snaps to a viewport edge, and persists its normalized position', () => {
  ...
  assert.match(readerSource, /function\s+capturePointerSafely|const\s+capturePointerSafely\s*=/);
  assert.match(readerSource, /hasPointerCapture/);
  assert.match(readerSource, /capturePointerSafely\(orb/);
  assert.match(readerSource, /capturePointerSafely\(handle/);
...
test('manual rescans await playback cancellation and only rebuild the document state', () => {
...
test('long-thread source locations are indexed on demand after extraction', () => {
  ...
  const scanStart = readerSource.indexOf('async function scanCurrentPage');
  const success = readerSource.indexOf('type: "LOAD_SUCCESS"', scanStart);
  const rendered = readerSource.indexOf('render();', success);
  assert.ok(scanStart >= 0 && success > scanStart && rendered > success);
  assert.doesNotMatch(readerSource.slice(success, rendered), /ensureSourceIndex\(/);
});

test('hidden author and voice tabs do not render during long-thread preparation', () => {
  const renderBody = readerSource.match(/function\s+render\(\)\s*\{([\s\S]*?)\n\s*\}/);
  assert.ok(renderBody, 'missing reader render function');
  assert.match(renderBody[1], /state\.tab\s*===\s*["']authors["']\)\s*renderAuthors\(\)/);
  assert.match(renderBody[1], /state\.tab\s*===\s*["']voices["']\)\s*renderVoices\(\)/);
  assert.doesNotMatch(renderBody[1], /renderNow\(\);\s*renderAuthors\(\);/);
});

test('collapsed orb is an accessible playback control with a live compact status', () => {
  assert.match(readerSource, /data-role="orb-status"[^>]*data-action="open-panel"/);
  assert.match(readerSource, /data-role="orb-status-label"[^>]*role="status"/);
  assert.match(readerSource, /aria-live="polite"/);
  assert.match(readerSource, /aria-pressed/);
  assert.match(readerSource, /aria-busy/);
  assert.match(readerSource, /function\s+currentOrbStatus/);
  assert.match(readerSource, /deriveOrbStatus/);
  assert.match(readerSource, /const\s+type\s*=\s*desiredPaused\s*\?\s*"tts:pause"\s*:\s*"tts:resume"/);
  assert.match(readerSource, /sendPlaybackControl\(type,\s*controlSession\)/);
  assert.match(readerSource, /state\.panelOpen/);
  const orb = cssRule('.qr-orb');
  assert.match(orb, /width:\s*44px/);
  assert.match(orb, /height:\s*44px/);
  const status = cssRule('.qr-orb-status');
  assert.match(status, /cursor:\s*pointer/);
  assert.match(readerCss, /\.qr-orb-status\.is-(?:error|offline)/);
});

test('reader surfaces a backend error even when the gateway is healthy', () => {
  assert.match(
    readerSource,
    /const\s+healthStates\s*=\s*new\s+Set\(\[providerState,\s*gateway,\s*backend\]/u,
  );
  assert.match(
    readerSource,
    /healthStates\][\s\S]{0,180}\["error",\s*"failed"\][\s\S]{0,160}setServiceStatus\("Provider 连接失败",\s*"error"\)/u,
  );
});

test('active sentence is a restrained translucent wash without the old text bloom', () => {
  ...
  const pageHighlight = pageHighlightCss.match(/::highlight\(qwen-reader-current\)\s*\{([\s\S]*?)\}/);
  assert.ok(pageHighlight, 'Missing page highlight rule');
  assert.match(pageHighlight[1], /background-color:\s*rgba\(116,\s*88,\s*232,\s*\.075\)/);
  assert.match(pageHighlight[1], /text-shadow:\s*none/);
  assert.doesNotMatch(pageHighlight[1], /box-shadow\s*:|border\s*:|outline\s*:|animation\s*:/);
  assert.doesNotMatch(pageHighlightCss, /qwen-reader-text-bloom/);
  assert.doesNotMatch(pageHighlightCss, /@keyframes\s+qr-highlight-sweep/);
  assert.deepEqual(manifest.content_scripts[0].css, ['content/page-highlight.css']);
  assert.doesNotMatch(readerSource, /createElement\(["']style["']\)|qwen-reader-page-style/);
});
...
test('motion is restrained and respects reduced-motion preferences', () => {
  ...
  assert.match(motionCss, /160ms|180ms|190ms|220ms|240ms/);
