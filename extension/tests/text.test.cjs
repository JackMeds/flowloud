...
test('buildSpeechWords returns Unicode word spans and maps them back around removed emoji', () => {
  const { prepareSpeech, buildSpeechWords } = loadTextModule();
  const prepared = prepareSpeech('你好👋世界，Hello world！');
  const words = buildSpeechWords(prepared, { locale: 'zh-CN' });

  assert.deepEqual(words.map((word) => word.text), ['你好', '世界', 'Hello', 'world']);
  assert.ok(words.every((word) => word.kind === 'word'));
  assert.ok(words.every((word) => word.speechEnd > word.speechStart));
  assert.ok(words.every((word) => word.sourceEnd > word.sourceStart));
  assert.deepEqual(
    words.map((word) => prepared.sourceText.slice(word.sourceStart, word.sourceEnd)),
    ['你好', '世界', 'Hello', 'world']
  );
  assert.ok(words.every((word, index) => index === 0 || (
    word.speechStart >= words[index - 1].speechEnd &&
    word.sourceStart >= words[index - 1].sourceEnd
  )));
});

test('buildSpeechWords excludes punctuation, whitespace, emoji and keycap graphemes', () => {
  const { prepareSpeech, buildSpeechWords } = loadTextModule();
  const prepared = prepareSpeech('版本 2️⃣ 已发布 👍🏽，真的！');
  const words = buildSpeechWords(prepared, { locale: 'zh-CN' });

  assert.deepEqual(words.map((word) => word.text), ['版本', '已', '发布', '真的']);
  assert.ok(words.every((word) => !/[，！\s]/u.test(word.text)));
  assert.ok(words.every((word) => !/[\p{Extended_Pictographic}\uFE0F\u20E3]/u.test(word.text)));
});

test('buildSpeechWords falls back to Latin grouping and Han graphemes without Intl.Segmenter', () => {
  const { prepareSpeech, buildSpeechWords } = loadTextModule();
  const prepared = prepareSpeech('你好世界 Hello world 123。');
  ...
  try {
    ...
    const words = buildSpeechWords(prepared);
    assert.deepEqual(words.map((word) => word.text), [
      '你', '好', '世', '界', 'Hello', 'world', '123'
    ]);
    assert.ok(words.every((word) => word.sourceStart !== null && word.sourceEnd !== null));
  } finally {
